...
with main_tabs[0]:
    ...
    with col_u2:
        ...
        st.image(f"https://ssl.pstatic.net/imgstock/chart3/world/day/^GSPC.png", caption="S&P 500 추이 미리보기", width=180)
    with col_u3:
        st.markdown(f'<div style="background-color:#f8f9fa; padding:12px; border-radius:6px; border-top:4px solid {x_color}; text-align:center; margin-bottom:5px;"><b>필라델피아 반도체 (^SOX)</b><br><span style="font-size:1.4em; font-weight:bold; color:{x_color};">{sox_val:,}</span> ({sox_chg:+.2f}%)</div>', unsafe_allow_html=True)
        st.image(f"https://ssl.pstatic.net/imgstock/chart3/world/day/^SOX.png", caption="필반도체 추이 미리보기", width=180)
    ...
    for idx, (code, name) in enumerate(target_stocks.items()):
        with stock_tabs[idx]:
            ...
            with col_chart:
                ...
                st.image(f"https://ssl.pstatic.net/imgstock/chart3/day/{code}.png", caption=f"{name} 시세 지표", use_container_width=True)
                ...
                if st.button(f"🔍 {name} AI 입체 분석 리포트 발행", key=f"btn_{code}", type="primary"):
                    with st.spinner(f"시스템이 {name}의 계량 지표와 글로벌 매크로 변수를 정밀 조립 중입니다..."):
                        try:
                            ...
                            # 🌟 주요 보강: 깨져있던 HTML 여는 태그 '<' 기호를 완벽하게 복구했습니다.
                            ...
                            with col_m1:
                                st.markdown(f"""<div style="background-color: {us_card_bg}; padding: 18px; border-radius: 8px; border-left: 5px solid #34495e;"><h4 style="margin-top:0; color:#2c3e50;">🇺🇸 미국 증시 연동값</h4><h2 style="margin: 10px 0; color:#2c3e50;">{us_impact_score} <span style="font-size:0.5em; color:#7f8c8d;">/ 100</span></h2><p style="font-size:0.9em; margin-bottom:0; color:#34495e;">{"✅ 미국 시장 상승 견인 적용" if nasdaq_chg >= 0 else "⚠️ 미국 시장 하방 압력 반영"}</p></div>""", unsafe_allow_html=True)
                            with col_m2:
                            ...
                            <table style="width:100%; border-collapse: collapse; margin: 10px 0; font-family: sans-serif; box-shadow: 0 2px 3px rgba(0,0,0,0.05); border-radius: 6px; overflow: hidden;">
                                ...
                                <tbody>
                                    <tr style="background-color: {us_row_color}; color: #2c3e50; font-weight: 500;">
                                        ...
                                        <td style="padding: 12px 15px; border: 1px solid #ddd;">{"✅ 미국 지수 동조화 온기 반영" if nasdaq_chg >= 0 else "⚠️ 미국 지수 하락 압력 연동"}</td>
                                    </tr>
                            ...
                            if today_disparity < 95:
                                ...
                                eval_text = "현재 주가는 20일 균형선을 이탈하여 딥 언더슈팅 국면에 진입했습니다. 실시간 연동된 미국 증시 데이터와 국내 이격도 왜곡 수치를 매칭한 결과, 매크로 공포가 반영된 저점 매수 구간입니다."
