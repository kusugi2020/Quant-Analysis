...
# 🌟 한글 에러가 나지 않도록 주석 처리를 완벽하게 지우고 영문으로만 세팅했습니다.
# 만약 개인 OpenAI API Key가 있으시다면 아래 쌍따옴표"" 사이에 넣어주시면 됩니다.
# 키가 없으셔도 스트림릿 Secrets 환경 변수에서 시스템이 자동으로 찾아옵니다.
            ...
            if st.button(f"🔍 {name} AI 입체 분석 리포트 발행", key=f"btn_{code}", type="primary"):
                with st.spinner(f"OpenAI가 {name}의 실시간 데이터와 마켓 이슈를 입체 분석 중입니다..."):
                    try:
                        ...
                        4. **이 종목만을 위한 냉정한 투자자 멘탈 가이드**: 이 종목의 고유한 변동성을 이겨내기 위한 날카롭고 고품격인 한 마디를 작성해줘.
                        """
