...
with menu_tab1:
    if is_data_loaded:
        ...
        # 2) 🛠️ 요청사항 반영: 이격도 차트 내 설정한 수치 빨간색 기준 가로선 생성
        disparity_chart_df = pd.DataFrame({
            "현재 이격도 흐름": chart_data["Disparity"],
            "내가 설정한 매수 기준선": input_threshold,  # 사용자가 지정한 수치로 면 채우기 및 기준선 형성
            "평균 기준선 (100%)": 100.0
        }, index=chart_data.index)
        
        # 이격도 그래프 출력 (내가 설정한 수치가 붉은 가이드라인 역할을 하도록 다중 컬럼 차트화)
        st.line_chart(disparity_chart_df, height=130, color=["#2563EB", "#EF4444", "#94A3B8"])
        st.caption(f"💡 **차트 읽는 법:** 파란색 이격도 곡선이 **빨간색 기준선({input_threshold}%)** 밑으로 뚫고 내려왔을 때가 통계 분석 대상이 되는 역사적 과매도 진입 시점입니다.")
        ...
        ### **📊 가중치가 크고 작은 판단 기준 바이블**
        
        **1. 숫자의 크기 (절대값) ➡️ 영향력의 세기**
        - 앞의 부호(+, -)를 완전히 배제하고 **숫자 자체의 크기(절대값)가 0에서 멀어질수록** 내일 반등할지 말지 확률 계산판을 크게 뒤흔드는 강력한 결정권자 지표입니다. 반대로 0에 아주 가까우면 지표가 아무리 요동쳐도 결과 예측에 아무런 영향을 주지 못하는 방관자 지표가 됩니다.
        - **예시 해석:** 만약 '20일 변동성' 가중치의 절대값이 타 지표에 비해 압도적으로 크다면, 단순히 많이 하락한 상태보다 최근 가격이 격렬하게 요동치며 **투자자들의 공포감이 극에 달한 상태**일수록 튕겨 오르는 용수철 탄력(알파)이 강하게 형성됨을 통계적으로 입증한 것입니다.
        
        **2. 부호의 의미 (+ 또는 -) ➡️ 영향력의 방향**
        - **플러스(+) 가중치 (정의 관계):** 해당 지표의 **수치가 커질수록** 반등 성공 확률이 올라갑니다. (ex. 거래량 이상치 가중치가 플러스라면, 당일 거래대금이 크게 폭발할수록 고래가 바닥에서 대량 매집하여 올렸을 확률이 높으므로 신뢰도가 대폭 증가함)
        - **마이너스(-) 가중치 (부의 관계):** 해당 지표의 **수치가 작아질수록** 반등 성공 확률이 올라갑니다. (ex. 이격도 가중치가 마이너스로 잡히면, 주가가 20일선 밑으로 깊숙하게 처박혀 숫자가 낮아질수록 반등 에너지가 응축됨)
        
        **3. 상대적 서열 비교법 (표준화 계수의 본질)**
        - 거래량(몇백만 단위), 이격도(90%대), 변동성(소수점 단위)은 원래 체급과 노는 물이 달라서 절대적 수치로 줄세우기를 할 수 없습니다. 
        - 인공지능 모델은 이를 완벽히 공평하게 만들기 위해 모든 변수를 평균 0, 표준편차 1로 스케일링한 후 계수를 추정합니다. 따라서 절대적인 합격선 점수가 정해져 있는 것이 아니라, 변수끼리 비교하여 **"어떤 지표가 다른 지표보다 상대적으로 몇 배나 더 정밀하게 반등에 관여하고 있는가"**의 상대 서열로 읽는 것이 퀀트 분석의 올바른 정석입니다.
