            ...
            if st.button(f"🔍 {name} AI 입체 분석 리포트 발행", key=f"btn_{code}", type="primary"):
                with st.spinner(f"AI가 {name}의 실시간 뉴스 및 퀀트 데이터를 통합 분석 중입니다..."):
                    try:
                        ...
                        제공된 [계량 데이터]와 너의 글로벌 경제 지식을 활용해 최신 정성적 이슈(실적, 업황 트렌드)를 매끄럽게 융합한 투자 보고서를 작성해줘. 판에 박힌 멘트는 지양해.
                        ...
                        # 🌟 외부 한도 제한이 걸리지 않는 전용 구글 오픈 인프라 주소 라우팅 연결
                        api_url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=" + "AQ.Ab8RN6LKQ7r0SVZ0i5_uYFDQb67iz6z0PUH0Dtp49DhWyOIK0w"
