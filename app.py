    ...
    for idx, (code, name) in enumerate(target_stocks.items()):
        with stock_tabs[idx]:
            ...
            if st.button(f"🔍 {name} AI 입체 분석 리포트 발행", key=f"btn_{code}", type="primary"):
                with st.spinner(f"알고리즘 시스템이 {name}의 데이터를 정밀 분석 중입니다..."):
                    try:
                        # 1. 네이버 금융 데이터 수집
                        ...
                        # 2. 고정형 정밀 퀀트 분석서 자동 조립 엔진 (서버 에러 방지용)
                        if today_disparity < 90:
                            status = "🚨 **단기 강력 과매도 (낙폭과대 구간)**"
                            eval_text = f"현재 {name}의 주가는 역사적 하단선인 이격도 {today_disparity}%까지 급락해 있습니다. 통계적으로 시장의 과도한 공포가 반영된 자리이며, 본질 가치 대비 낙폭이 지나치게 깊어진 대반등 임박 구간입니다."
                            buy_strategy = "⭐⭐⭐⭐⭐ (5 / 5) - 적극적 분할 매수 및 물타기 최적기. 용수철 탄성력이 상방으로 최대치로 압축된 타이밍입니다."
                            sell_strategy = "🚨 손절 절대 금지 구간. 과거 데이터상 이 포지션에서의 매도는 실익이 없으며 반등을 기다리는 끈기가 필요합니다."
                        elif today_disparity < 98:
                            status = "📉 **단기 조정 및 매수 우위 구간**"
                            eval_text = f"현재 {name}의 주가는 20일 이평선 아래에서 숨고르기를 진행 중입니다. 지지선을 견고하게 다지며 에너지를 응축하는 건강한 조정 단계로 해석됩니다."
                            buy_strategy = "⭐⭐⭐⭐ (4 / 5) - 점진적 비중 확대 유효. 단기 매물이 충분히 소화된 안정적인 진입 포지션입니다."
                            sell_strategy = "자잘한 흔들림에 물량을 털리기보다 장기 보유 관점을 굳건히 유지하는 관망 전략이 스마트합니다."
                        elif today_disparity <= 103:
                            status = "⚖️ **적정 주가 및 수렴 횡보 구간**"
                            eval_text = f"현재 {name}의 주가는 한 달간의 정당한 균형 가격대인 {today_disparity}%선에 바짝 붙어 있습니다. 상방과 하방의 팽팽한 힘겨루기가 지속되는 지점입니다."
                            buy_strategy = "⭐⭐⭐ (3 / 5) - 방향성이 모호한 중립 구간이므로 섣부른 추격 매수보다는 기존 비중을 유지하십시오."
                            sell_strategy = "종목 교체나 자금 확보 목적이 아니라면 추세 돌파가 확실해질 때까지 자산을 지키며 관망하는 것이 유리합니다."
                        else:
                            status = "🔥 **단기 과열 및 고점 경계 구간**"
                            eval_text = f"현재 {name}의 주가는 이격도 {today_disparity}%로 시장 참여자들의 탐욕과 단기 오버슈팅이 최고조에 달한 오버영역에 진입했습니다."
                            buy_strategy = "⭐ (1 / 5) - 진입 금지. 추격 매수 시 단기 고점에 물리 리스크가 매우 큽니다. 낙하산 없이 뛰어드는 격입니다."
                            sell_strategy = "⭐⭐⭐⭐⭐ (5 / 5) - 분할 익절 시작. 탐욕을 버리고 리스크 관리 차원에서 수익금을 챙겨 현금을 확보해 두십시오."
                        ...
                        * **입체 진단:** {eval_text}
                        ...
                        #### 2. 신규 및 추가 매수(물타기) 전략
                        * **추천 점수 및 지침:** {buy_strategy}
                        
                        #### 3. 리스크 관리 및 매도 전략
                        * **대응 지침:** {sell_strategy}
                        
                        #### 4. 변동성 장세 멘탈 이정표
                        > 💡 *“대형주 시장에서 이격도가 무너졌을 때 회귀 본능에 의해 20일 균형선으로 강하게 복귀할 확률은 언제나 압도적이었습니다. 외부의 잡음에 흔들리지 말고 숫자가 주는 통계의 힘을 믿으십시오.”*
                        """)
